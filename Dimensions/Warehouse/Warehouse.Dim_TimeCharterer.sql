...
Description:	Creates the Warehouse.Dim_TimeCharterer table
Changes
...
drop table if exists Warehouse.Dim_TimeCharterer;
go

create table Warehouse.Dim_TimeCharterer
	(
		TimeChartererKey				int					not null identity(1, 1),
		...
		constraint [PK_Warehouse_Dim_TimeCharterer_Key] primary key clustered 
		(
			TimeChartererKey asc
		)
			with 
